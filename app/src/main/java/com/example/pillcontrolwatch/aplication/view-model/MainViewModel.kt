    ...
    init {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (pendingReset) {
                ...
                Log.d("MainViewModel", "MainViewModel: if (pendingReset) part : $pendingReset")
            } else {
                // For normal login flows, ensure any stale pending flag is cleared.
                prefs.edit().remove("pending_reset_password").apply()
                Log.d("MainViewModel", "MainViewModel: else {} part: $pendingReset")
                // End NEW CODE
            }
            ...
            supabaseClient.auth.sessionStatus.collect { status ->
                when (status) {
                    is SessionStatus.Authenticated -> {
                        ...
                        Log.d("Auth", "Authenticated: Access Token: $token")
                    ...
                    SessionStatus.Initializing -> {
                        Log.d("Auth", "Auth is initializing")
                        _toastMessageFlow.emit("Auth is initializing")
                    }
                    is SessionStatus.RefreshFailure -> {
                        Log.d("Auth", "Session refresh failed: ${status.cause}")
                        _toastMessageFlow.emit("Session refresh failed: ${status.cause}")
                    }
                    else -> {
                        Log.d("Auth", "Other auth state: $status")
                    }
                }
            }
        }
